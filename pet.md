...
bash# На выделенном NFS сервере (192.168.10.50)
...
bash# VM: postgresql-primary (192.168.10.40)
...
bash# VM: gitlab-primary (192.168.10.10)
...
bash# VM: sonarqube (192.168.10.30)
# Specs: 4 vCPU, 8GB RAM, 100GB SSD
...
bash# На k3s-master-1 (192.168.20.10)
...
bash# На k3s-master-2 (192.168.20.11)
...
bash# На всех worker нодах (192.168.20.21-23)
export K3S_TOKEN="<token-from-master-1>"
...
bash# Установка зависимостей на всех нодах
...
bash# На haproxy-1 (192.168.30.10) и haproxy-2 (192.168.30.11)
...
bash# На haproxy-1 (192.168.30.10) - MASTER
...
bash# На локальной машине или bastion host
...
deploy-k8s:
  ...
  script:RetryAContinuebash    - kubectl config set-cluster k3s --server=https://192.168.20.10:6443 --insecure-skip-tls-verify=true
...
bash# Через Nexus UI (http://192.168.30.1:8081)
# 1. Login as admin
...
bash# В SonarQube UI (http://192.168.30.1:9000)
# 1. Login as admin
...
bash# Создаем ServiceMonitor для PetClinic
cat << EOF | kubectl apply -f -
apiVersion: v1
kind: Service
metadata:
...
bash# На GitLab VM устанавливаем node_exporter
cd /tmp
...
bash# На GitLab VM создаем скрипт бэкапа
...
bash# Установка cert-manager в K8s
...
bash# Базовая Network Policy для изоляции namespaces
cat << EOF | kubectl apply -f -
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
...
bash# Включаем Pod Security Admission
cat << EOF | kubectl apply -f -
apiVersion: v1
kind: Namespace
metadata:
...
9.1 Smoke Tests
bash# Создаем скрипт для проверки всех компонентов
...
9.2 Load Testing
bash# Установка k6 для load testing
apt install -y gpg
...
bash# Тестируем HAProxy failover
...
echo "Failover test completeRetryAContinuebashecho "Failover test complete"
EOF
...
bash# Создаем директорию для документации в GitLab
...
bashcat << 'EOF' > /tmp/production-checklist.md
...
test_component "Loki operational" "kubectl get pods -n monitoring -l app.kubernetes.io/name=loki -oRetryAContinuebashjsonpath='{.items[*].status.phase}' | grep -q Running"
...
bash# Создаем скрипт для сбора базовых метрик
...
bash# Создаем AlertManager configuration
...
bash# Создаем скрипт для экспорта всей конфигурации
...
bash# Финальный go-live checklist script
...
Daily Operations
bash# Создаем скрипт для ежедневных проверок
