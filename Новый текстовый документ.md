...
1.1 Требования к VM
VMCPURAMDiskOSGitLab48GB50GBUbuntu 22.04K3s Master24GB40GBUbuntu 22.04K3s Worker-128GB60GBUbuntu 22.04K3s Worker-228GB60GBUbuntu 22.04HAProxy12GB20GBUbuntu 22.04
...
На Proxmox хосте:
bash# Скачать Ubuntu Cloud Image
...
bash# Редактирование конфигурации
sudo nano /etc/gitlab/gitlab.rb
Измените следующие параметры:
rubyexternal_url 'http://gitlab.local.lab'
...
spec:
  ...
  - default
Примените:
bashkubectl apply -f metallb-config.yaml
...
postgresql:
  ...
  persistence:
    ...
    size: 10Gi
5.3 Установка
bashkubectl create namespace sonarqube
...
bashkubectl wait --for=condition=ready pod \
  -l app=sonarqube \
  -n sonarqube \
  --timeout=600s
Доступ:
...
bashhelm repo add sonatype https://sonatype.github.io/helm3-charts/
helm repo update
...
6.3 Установка
bashkubectl create namespace nexus
...
KeyValueMaskedProtectedNEXUS_USERadminNoNoNEXUS_PASSWORD(ваш пароль)YesNoSONAR_HOST_URLhttp://10.0.10.101:9000NoNoSONAR_TOKEN(токен из SonarQube)YesNoCI_REGISTRYhttps://index.docker.io/v1/NoNoCI_REGISTRY_USER(Docker Hub username)NoNoCI_REGISTRY_PASSWORD(Docker Hub token)YesNoKUBECONFIG(содержимое ~/.kube/config)NoNo
Для KUBECONFIG:
...
xml<nexus.host.url>http://10.0.10.102:8081</nexus.host.url>
...
propertiessonar.projectKey=spring-petclinic
...
bashmkdir -p petclinic-chart/templates
...
git add .
git commit -mRetryClaude does not have the ability to run the code it generates yet.AContinuebashgit commit -m "Initial commit with CI/CD pipeline"
...
bashkubectl create namespace gitlab-runner
...
bash# Проверьте Docker Hub credentials
...
Реализуйте Blue-Green или Canary deployment
Добавьте security scanning (Trivy, OWASP Dependency Check)
