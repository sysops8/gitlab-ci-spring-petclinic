...
|----|-----|-----|------|-----------|
| Gateway | 2 | 4GB | 20GB | NAT, DNS (BIND), HAProxy, Jump host |
| GitLab | 4 | 8GB | 50GB | Git repository, CI/CD orchestration |
| K3s Master | 2 | 4GB | 40GB | Kubernetes control plane |
| K3s Worker-1 | 2 | 8GB | 60GB | Kubernetes workloads (Nexus) |
| K3s Worker-2 | 2 | 8GB | 60GB | Kubernetes workloads (SonarQube) |
...
```bash
# Переход в директорию шаблонов
cd /var/lib/vz/template/iso

# Загрузка Ubuntu Cloud Image
wget https://cloud-images.ubuntu.com/jammy/current/jammy-server-cloudimg-amd64.img

# Создание VM для шаблона (ID 9000)
qm create 9000 \
...
# Добавление Cloud-Init диска
qm set 9000 --ide2 local-lvm:cloudinit

# Настройка загрузки
qm set 9000 --boot c --bootdisk scsi0

# Настройка serial консоли
qm set 9000 --serial0 socket --vga serial0

# Включение QEMU Guest Agent
qm set 9000 --agent enabled=1

# Увеличение размера диска до 20GB (опционально)
...
Создайте директорию проекта на вашей рабочей машине:

```bash
mkdir -p ~/proxmox-petclinic-infra
cd ~/proxmox-petclinic-infra
```

Создайте `providers.tf`:
...
```hcl
locals {
  ssh_keys = file(pathexpand(var.ssh_public_key))
}

# Gateway VM (двойной интерфейс)
...
output "gateway_external_ip" {
  value       = "10.0.10.30"
  description = "Gateway external IP (access point)"
}

output "gateway_internal_ip" {
  value       = "192.168.50.1"
  description = "Gateway internal IP"
}

output "gitlab_ip" {
  value       = "192.168.50.10"
  description = "GitLab VM IP"
}
...
output "access_instructions" {
  value = <<-EOT
    ...
    2. Внутренние VM (через jump):
       ssh -J ubuntu@10.0.10.30 ubuntu@192.168.50.XX
    
    3. Web интерфейсы (после настройки DNS/HAProxy):
       http://gitlab.local.lab
       http://nexus.local.lab
       http://sonarqube.local.lab
       http://petclinic.local.lab
    
    4. Настройте DNS на клиенте: 10.0.10.30
       или добавьте в hosts:
       10.0.10.30 gitlab.local.lab nexus.local.lab sonarqube.local.lab petclinic.local.lab
  EOT
}
```
...
### 1.5 Развертывание инфраструктуры
...
**Важно**: Дождитесь полной инициализации cloud-init (2-3 минуты) перед подключением.
...
# С gateway проверка внутренних VM
...
```
ens18: 10.0.10.30/24 (внешний)
ens19: 192.168.50.1/24 (внутренний)
```

**Важно**: Имена интерфейсов могут отличаться (eth0, enp0s18 и т.д.). Используйте актуальные имена в дальнейших командах.
...
EXT_IF="ens18"  # Внешний интерфейс - замените на свой!
INT_IF="ens19"  # Внутренний интерфейс - замените на свой!
...
### 5.2 Установка Helm на Windows (для управления кластером)
...
1. Скачайте: https://github.com/helm/helm/releases
2. Распакуйте `helm.exe` в `C:\Program Files\helm\`
3. Добавьте в PATH
4. Проверка: `helm version`
...
На Windows создайте `C:\Users\YourUser\.kube\config`:
...
Вставьте содержимое, но **измените**:
...
**Важно**: Для доступа с Windows потребуется маршрутизация к 192.168.50.0/24. Если нет прямой маршрутизации, используйте:

**Вариант 1: Port forwarding через SSH**

```powershell
# В отдельном терминале (оставьте открытым)
ssh -L 6443:192.168.50.20:6443 ubuntu@10.0.10.30 -N

# В другом терминале
kubectl get nodes  # Будет работать через туннель
```

**Вариант 2: Статический маршрут на Windows**

```powershell
# От администратора
route add 192.168.50.0 mask 255.255.255.0 10.0.10.30 -p
...
Вы построили полнофункциональную DevOps платформу enterprise-уровня на собственном оборудовании. Эта инфраструктура:
...
1. Изучите метрики и логи вашей системы
2. Экспериментируйте с различными приложениями
3. Расширяйте функциональность постепенно
4. Документируйте все изменения
5. Делитесь опытом с сообществом

Помните: это только начало вашего DevOps путешествия. Продолжайте учиться, экспериментировать и совершенствовать свои навыки!
